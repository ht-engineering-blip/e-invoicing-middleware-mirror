...
export const onlyAdmin = (auth: AuthContext, customMessage?: string) => {
  ...
  if (!auth?.isAdmin) {
    throw new ForbiddenError(
      customMessage || "Forbidden: You do not have access to this tenant",
    );
  }
};
...
export const onlyTenantAdmin = (auth: AuthContext, tenantId: string) => {
  ...
  if (auth?.isTeamMember) {
    ...
    if (role !== "owner" && role !== "admin") {
      throw new ForbiddenError(
        "Forbidden: You do not have the required administrative role for this tenant",
      );
    }
  }
};
