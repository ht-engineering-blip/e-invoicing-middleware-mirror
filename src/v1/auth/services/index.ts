...
export class AuthService {
     ...
     */
    async createAuthToken(tenant: TenantDocument & { type: string, role: TeamMemberRole }, expiresIn?: string): Promise<string> {
        const tokenPayload = {
            ...tenant,
    ...
    async requestPasswordReset(email: string): Promise<{ success: boolean; message: string }> {
        try {
            ...
            // Send reset email
            await this.sendPasswordResetEmail(tenant, resetToken, tenant.businessName);

            logger.info('Password reset token created', { email, tenantId: tenant.tenantId });
    ...
    private async sendPasswordResetEmail(tenant: TenantDocument, token: string, businessName: string): Promise<void> {
        try {
            ...
            logger.error('Failed to send password reset email', { email: tenant.contactEmail, error: error.message });
     ...
     */
    private async sendPasswordChangedEmail(tenant: TenantDocument): Promise<void> {
        try {
            let emailBody: MailContent = {
            ...
            logger.info('Password changed notification sent', { email: tenant.contactEmail });
        } catch (error: any) {
            logger.error('Failed to send password changed notification', { email: tenant.contactEmail, error: error.message });
        }
    }
}
