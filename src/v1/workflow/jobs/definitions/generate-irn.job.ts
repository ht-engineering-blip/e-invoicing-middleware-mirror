...
import { generateInvoiceRef, generateIRN } from "../../utils/transformer/utils";
...
export function registerGenerateIrnJob(): void {
  agenda.define("workflow:generate-irn", async (job: Job<JobChainData>) => {
    ...
    try {
      ...
      console.log({
        ...
        issueDate: context?.originalPayload?.invoice?.issueDate,
      });
      const irn =
        existingInvoice ||
        generateIRN(
          ...
          context?.originalPayload?.invoice?.issueDate || new Date(),
        );
