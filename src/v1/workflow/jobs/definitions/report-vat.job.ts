  ...
  agenda.define(
    ...
    async (job: Job<JobChainData>) => {
      ...
      try {
        ...
        const vatData = context.vatReportData ?? context.originalPayload?.vatReportData;
        if (!vatData) {
          throw new Error('VAT report data not found in context or originalPayload.vatReportData');
        }
        ...
        logger.info('[Job:report-vat] Reported', { jobChainId, irn: context.irn });
