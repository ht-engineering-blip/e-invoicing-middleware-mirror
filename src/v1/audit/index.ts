...
export const auditRoutes = new Elysia({ prefix: '/audit' })
  ...
  .get('/resource/:resourceType/:resourceId', async ({ auth, params, auditService }) => {
    onlyAdmin(auth!, 'Forbidden: Admin access required');
    const logs = await auditService.getResourceAuditTrail(params.resourceType, params.resourceId);
