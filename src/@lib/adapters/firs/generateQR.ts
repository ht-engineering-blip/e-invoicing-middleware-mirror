...
export async function generateQRCode({
  ...
  try {
    ...
    try {
      ...
      debug('Encryption completed successfully, buffer length: %d', encryptedBuffer.length);
    ...
    debug('Converted encrypted data to base64, length: %d', encryptedBase64.length);
    ...
    const backgroundSquare = await sharp.default({
      create: {
        width: bgSize,
        height: bgSize,
        channels: 4,
    ...
    const logoBuffer = await sharp.default(backgroundSquare)
      .composite([{
        input: await sharp.default(logoBufferRaw)
    ...
    const composedBuffer = await sharp.default(qrCodeBuffer)
      .composite([{
        input: logoBuffer,
        gravity: 'center'
      }])
