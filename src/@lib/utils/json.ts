...
/**
 * Parses JSON with robust cleaning and error handling
 * @param {string} dirtyJson - Potentially dirty JSON string
 * @param {Object} options - Cleaning options
 * @returns {Object} Parsed JSON object
 * @throws {Error} If JSON cannot be parsed even after cleaning
 */
function parseDirtyJson(dirtyJson: any, options = {}) {
    try {
        ...
        try {
            // Clean and try again
            const cleaned = cleanJsonString(dirtyJson, options);
            return JSON.parse(cleaned);
        } catch (cleanError: any) {
            // If still failing, try more aggressive cleaning
            try {
                const aggressiveCleaned = cleanJsonString(dirtyJson, {
                    ...options,
