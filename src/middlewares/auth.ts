 ...
 */
export const requireAdmin = (instance: Elysia) =>
...
export const requireApiKey = (instance: Elysia) =>
  instance.resolve(async ({ headers }): Promise<{ auth: AuthContext }> => {
    ...
    if (apiKeyDoc.expiresAt && apiKeyDoc.expiresAt < new Date()) {
      ...
      apiKeyRepo
        .revoke(apiKeyDoc._id.toString(), "Expired", "system")
        .catch(() => {});
    ...
    async ({ headers, request }): Promise<{ auth: AuthContext }> => {
      ...
      if (apiKey) {
        ...
        if (
          ...
          let decryptedClientID = decryptSensitiveData(
            tenant.config.firsCredentials.clientId,
          );
      ...
      if (authHeader && authHeader.startsWith("Bearer ")) {
        ...
        try {
          ...
          if (decoded.type === "team_member") {
            ...
            const teamMember = await teamMemberRepo.findByUserId(
              decoded.userId,
            );
            ...
            if (teamMember.status !== TeamMemberStatus.ACTIVE) {
              throw new UnauthorizedError(
                `Team member account is ${teamMember.status}`,
              );
            }
          ...
          if (decoded?.purpose && decoded?.purpose == "set-password") {
            ...
            if (
              ...
              pathname !== "/v1/auth/set-password"
            ) {
              throw new UnauthorizedError(
                "set-password token is only authorized for setting password",
              );
            }
      ...
      throw new UnauthorizedError(
        "Authentication required. Provide x-admin-key, x-api-key or Authorization header",
      );
    },
  );
 ...
 */
export const optionalAuth = (instance: Elysia) =>
  instance.resolve(async ({ headers }): Promise<{ auth?: AuthContext }> => {
    const apiKey = headers["x-api-key"];
    const authHeader = headers["authorization"];
        ...
        if (apiKeyDoc && apiKeyDoc.status === "active") {
          if (!apiKeyDoc.expiresAt || apiKeyDoc.expiresAt >= new Date()) {
            ...
            if (tenant && tenant.status === "active") {
              apiKeyRepo
                .updateLastUsed(apiKeyDoc._id.toString())
                .catch(() => {});
