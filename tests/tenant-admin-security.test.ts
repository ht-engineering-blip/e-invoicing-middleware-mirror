...
describe("Tenant Admin Routes & Credentials Redaction Security Tests", () => {
  ...
  beforeAll(async () => {
    const routesMod = await import("../src/v1/tenants/routes/admin.routes");
    adminTenantRoutes = routesMod.default;

    const tenantServiceMod =
    ...
    const memberRepoMod =
      await import("../src/v1/tenants/repos/team-member.repo");
    ...
    getERPSyncConfigSpy = spyOn(
      TenantService.prototype,
      "getERPSyncConfig",
    ).mockImplementation(async function (this: any, tenantId: string) {
    ...
    createTenantSpy = spyOn(
      TenantService.prototype,
      "createTenant",
    ).mockImplementation(async function (this: any, body: any, actor?: any) {
      if (
    ...
    listTenantsSpy = spyOn(
      TenantService.prototype,
      "listTenants",
    ).mockImplementation(async function (this: any, filters?: any) {
      if (
        filters?.tenantId &&
    ...
    findByTenantIdSpy = spyOn(
      ...
      if (id === "tenant-other")
        return { tenantId: "tenant-other", status: "active" } as any;
    ...
    findOneSpy = spyOn(
      ...
      if (
        query?.tenantId?._eq === "tenant-123" ||
        query?.tenantId?._eq === "tenant-other"
      ) {
    ...
    updateSpy = spyOn(TenantRepository.prototype, "update").mockImplementation(
      async function (this: any, tenantId: string, data: any) {
    ...
    findByUserIdSpy = spyOn(
      TeamMemberRepository.prototype,
      "findByUserId",
    ).mockImplementation(async function (this: any, userId: string) {
