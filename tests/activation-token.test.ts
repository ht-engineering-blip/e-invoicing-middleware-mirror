...
import { publicOnboardingRoutes, protectedOnboardingRoutes } from '../src/v1/tenants/routes/onboarding.routes';
...
describe('Onboarding Activation Token Lifecycle', () => {
  ...
  beforeAll(() => {
    ...
    updateSpy = spyOn(TenantRepository.prototype, 'update').mockImplementation(async function(this: any, id: string, data: any) {
    ...
    notifyTenantSpy = spyOn(TenantService.prototype, 'notifyTenant').mockImplementation(async function(this: any, mail: any, tenant: any) {
  ...
  describe('TenantService Activation Token Helpers', () => {
    ...
    it('should correctly parse activation token expiry without ternary operators', () => {
      ...
      expect(tenantService.getActivationTokenExpiry({ metadata: {} })).toBeNull();
    });
    ...
    it('should correctly validate activation token', () => {
      ...
      expect(tenantService.isActivationTokenValid(validTenant, 'token-123')).toBe(true);
      expect(tenantService.isActivationTokenValid(validTenant, 'wrong-token')).toBe(false);
      ...
      expect(tenantService.isActivationTokenValid(expiredTenant, 'token-123')).toBe(false);
      expect(tenantService.isActivationTokenValid(null, 'token-123')).toBe(false);
    });
    ...
    it('should correctly verify token timeframe', () => {
      ...
      expect(tenantService.isActivationTokenInTimeframe(activeTenant)).toBe(true);
      ...
      expect(tenantService.isActivationTokenInTimeframe(expiredTenant)).toBe(false);
