...
describe("FIRS Credit Note Invoicing and Validation", () => {
  ...
  const validLineItems = [
    {
      ...
      invoiced_quantity: 2,
      line_extension_amount: 600000.0,
      item: {
  ...
  const validMonetaryTotal = {
    line_extension_amount: 600000.0,
    tax_exclusive_amount: 600000.0,
    tax_inclusive_amount: 645000.0,
    payable_amount: 645000.0,
  };
  ...
  describe("Zod Schema Validation", () => {
    ...
    it("should fail to validate a credit note (381) without billing_reference", () => {
      ...
      if (!result.success) {
        const errors = result.error.flatten();
        expect(errors.fieldErrors.billing_reference).toBeDefined();
        expect(errors.fieldErrors.billing_reference?.[0]).toContain(
          "billing_reference is required",
        );
      }
    });
  ...
  describe("process_credit_note Agenda Job", () => {
    it("should process a credit note job by cloning the original invoice and setting credit note fields", async () => {
      const { registerProcessCreditNoteJob } =
        await import("../src/v1/workflow/jobs/definitions/process-credit-note.job");
      ...
      expect(lastNextOutput.transformedInvoice.irn).toBe(
        "CN0001-6AFCD0BD-20260411",
      );
      expect(lastNextOutput.transformedInvoice.invoice_reference).toBe(
        "CN-9999",
      );
