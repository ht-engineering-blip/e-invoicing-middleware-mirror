...
describe("Webhook Events Search & Filter Tests", () => {
  ...
  beforeAll(async () => {
    ...
    countSpy = spyOn(
      ...
      const results = await WebhookEventRepository.prototype.find.call(
        {} as any,
        query,
      );
  ...
  it("filters by irn with partial substring match and updates meta.total", async () => {
    const res = await app.handle(
      makeReq("http://localhost/webhook/events?irn=F1DFF&page=1&limit=10"),
    );
  ...
  it("filters by search across irn, eventId, and eventType", async () => {
    // Search by eventId
    const resId = await app.handle(
      makeReq("http://localhost/webhook/events?search=wh_evt_003"),
    );
    ...
    const resType = await app.handle(
      makeReq("http://localhost/webhook/events?search=invoice.created"),
    );
    ...
    // Search by irn
    const resIrn = await app.handle(
      makeReq("http://localhost/webhook/events?search=F1DFF"),
    );
  ...
  it("returns empty result when no match found", async () => {
    const res = await app.handle(
      makeReq("http://localhost/webhook/events?irn=NON_EXISTENT"),
    );
    const json = await res.json();
    expect(json.success).toBe(true);
